...
// Ensure to include <Windows.h> first to get standard includes. It is not
// included here as Windows applications should include it themselves at the
// very beginning
...
// CPUIDDLL_EXPORTS symbol defined on the command line. this symbol should not
// be defined on any project that uses this DLL. This way any other project
// whose source files include this file see CPUIDDLL_API functions as being
...
CPUIDDLL_API int APIENTRY cpuid(DWORD eax, DWORD ecx, LPDWORD peax,
                                LPDWORD pebx, LPDWORD pecx, LPDWORD pedx);
...
/// <param name="bytes">
/// Number of bytes allocated by <paramref name="info"/>.
/// </param>
/// <returns>Number of elements put into <paramref name="info"/>.</returns>
/// <remarks>
/// On Windows XP, it's not possible to know on what processor the current
/// thread is running, so it will always execute on the first processor. To
/// specify which processor it should be run on, use the method <see
/// cref="iddumponcore"/>. On Windows Server 2003 and later, the thread affinity
/// is set to the current processor, but this is generally indeterminate. Again,
/// it's advised to use <see cref="iddumponcore"/>.
/// </remarks>
...
/// </param>
/// <returns>
/// Number of elements put into <paramref name="info"/>.
/// </returns>
CPUIDDLL_API int APIENTRY iddumponcore(struct cpuidinfo *info, size_t bytes,
                                       int core);
...
/// Number of elements put into <paramref /// name="info"/>.
/// </returns>
/// <remarks> The array buffer provided, <paramref name="info"/> is filled for
/// all cores. The first element is a fake CPUID entry, of veax being 0xFFFFFFFF
/// and vecx being the core number.
/// </remarks>
